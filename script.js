(function(){
  ...
  // ---------- date HUD ----------
  const d = new Date();
  hudDate.textContent = "'" + String(d.getFullYear()).slice(2) + " " + (d.getMonth()+1) + " " + d.getDate();

  // ---------- camera init ----------
  let isFrontFacing = false; // true => raw feed comes in flipped, needs correcting
  ...
  async function initCamera(){
    try{
      ...
      // some devices only expose a front camera even when 'environment' is requested;
      // its raw feed is flipped, so un-mirror it for both preview and saved photos
      isFrontFacing = settings.facingMode === 'user' || !settings.facingMode;
  ...
  function contrast(v, factor){ return (v - 128) * factor + 128; }
  ...
  function applyFilter(ctx, w, h, filterName, flash){
    ...
    let recipe;
    if (filterName === 'cybershot')  recipe = { blur:0.22, ca:1, grain:22, lift:14, warmR:22, warmG:6,  warmB:-14, cFactor:1.28, sat:1.18, vAlpha:0.42, vRadius:0.62 };
    else if (filterName === 'y2kflash') recipe = { blur:0.04, ca:0, grain:16, lift:0,  warmR:-18,warmG:-4, warmB:30,  cFactor:1.6,  sat:0.95, vAlpha:0.7,  vRadius:0.42 };
    else if (filterName === 'vhs')      recipe = { blur:0.16, ca:2, grain:18, lift:10, warmR:-4, warmG:10, warmB:6,   cFactor:1.15, sat:0.85, vAlpha:0.26, vRadius:0.78 };
    else                                 recipe = { blur:0.22, ca:1, grain:32, lift:6,  warmR:26, warmG:10, warmB:-4,  cFactor:1.08, sat:1.28, vAlpha:0.3,  vRadius:0.72 }; // disposable
    ...
    for (let i=0; i<data.length; i+=4){
      ...
      r = contrast(r, recipe.cFactor) + recipe.warmR + recipe.lift * (1 - r/255) * 0.4;
      g = contrast(g, recipe.cFactor) + recipe.warmG + recipe.lift * (1 - g/255) * 0.4;
      b = contrast(b, recipe.cFactor) + recipe.warmB + recipe.lift * (1 - b/255) * 0.4;

      if (flash){ r += 22; g += 18; b += 14; }
    ...
    const vg = ctx.createRadialGradient(w/2,h/2, h*0.18, w/2,h/2, h*recipe.vRadius);
    ...
    if (filterName === 'vhs'){
      ...
      for (let y=0; y<h; y+=3){ ctx.fillRect(0,y,w,2); }
    }
    ...
    if (filterName === 'disposable' || filterName === 'cybershot'){
      ...
      const lg = ctx.createRadialGradient(w*0.06, h*0.04, 0, w*0.06, h*0.04, w*leakR);
      const leakColor = filterName === 'disposable' ? '255,150,70' : '255,190,120';
    ...
    if (filterName === 'y2kflash'){
      ctx.save();
      ctx.globalCompositeOperation = 'screen';
      const fg = ctx.createRadialGradient(w*0.5, h*0.42, 0, w*0.5, h*0.42, w*0.55);
    ...
    const dt = new Date();
    const stamp = "'" + String(dt.getFullYear()).slice(2) + " " + (dt.getMonth()+1) + " " + dt.getDate();
  ...
  saveGalleryBtn.addEventListener('click', async ()=>{
    ...
    showToast(shared ? 'pick "save" in the share sheet!' : 'sharing isn\'t supported here — use download instead');
  });
  ...
  function backToLiveView(){
    ...
    if (cameraReady){ video.style.display = 'block'; }
  ...
  function renderFilmstrip(){
    filmstrip.innerHTML = '';
    if (photos.length === 0){
      filmstrip.innerHTML = '<span class="film-empty">no shots yet — your gallery will fill up here 🌸</span>';
    ...
    galleryPeekBtn.innerHTML = '<img src="' + photos[0].dataURL + '" style="width:100%;height:100%;object-fit:cover;border-radius:50%;">';
  }
  ...
  const hostStorage = (typeof window !== 'undefined' && window.storage) ? window.storage : null;
  ...
  async function requestPersistentStorage(){
    ...
    }catch(e){ console.error('persist() request failed', e); }
  }
  ...
  async function storeSet(key, value, shared){
    if (hostStorage){
      try{ return await hostStorage.set(key, value, shared); }catch(e){ console.error('host storage set failed', e); }
    }
    try{
    ...
    }catch(e){ console.error('indexedDB set failed', e); storageBroken = true; return null; }
  }
  async function storeGet(key, shared){
    if (hostStorage){
      try{ return await hostStorage.get(key, shared); }catch(e){ return null; }
    }
    try{
      const db = await openDB();
      return await new Promise((resolve, reject)=>{
        ...
        req.onsuccess = ()=> resolve(req.result ? { key, value: req.result.value, shared } : null);
        req.onerror = ()=> reject(req.error);
      });
    }catch(e){ return null; }
  }
  async function storeDelete(key, shared){
    if (hostStorage){
      try{ return await hostStorage.delete(key, shared); }catch(e){ return null; }
    }
    try{
      const db = await openDB();
      await new Promise((resolve, reject)=>{
  ...
  async function storeList(prefix, shared){
    if (hostStorage){
      try{ return await hostStorage.list(prefix, shared); }catch(e){ return null; }
    }
    try{
      const db = await openDB();
  ...
  async function saveToDeviceGallery(dataURL, filename){
    try{
      ...
      if (navigator.canShare && navigator.canShare({ files: [file] })){
        await navigator.share({ files: [file], title: 'mochi cam photo' });
        return true;
      }
    }catch(e){
      // user closing the share sheet without picking anything also lands
      // here (AbortError) — that's not a real failure, just don't retry
  ...
  lbDelete.addEventListener('click', async ()=>{
    ...
    renderFilmstrip();
    closeLightbox();
    try{ await storeDelete('photo:' + id, false); }catch(err){ /* ignore */ }
  });
  ...
  async function savePhoto(dataURL){
    ...
    if (photos.length > 30){
      ...
      try{ await storeDelete('photo:' + removed.id, false); }catch(err){ /* ignore */ }
    }
    renderFilmstrip();
    const result = await storeSet('photo:' + id, JSON.stringify({ dataURL, ts: id }), false);
      ...
      for (const key of listRes.keys){
        try{
          ...
          if (res && res.value){
            ...
            loaded.push({ id: key.replace('photo:',''), dataURL: parsed.dataURL, ts: parsed.ts });
          }
